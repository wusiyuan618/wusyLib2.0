package com.wusy.wusylibrary.activity.uploadFile.localAum.ui;
...
import com.wusy.wusylibrary.activity.uploadFile.localAum.AppManager;
import com.wusy.wusylibrary.activity.uploadFile.localAum.common.ExtraKey;
import com.wusy.wusylibrary.activity.uploadFile.localAum.common.LocalImageHelper;
import com.wusy.wusylibrary.activity.uploadFile.localAum.widget.AlbumViewPager;
import com.wusy.wusylibrary.activity.uploadFile.localAum.widget.MatrixImageView;
                    ...
                    public void run() {
                        if (folders != null) {
                            ...
                            if (checkedItems.size()+LocalImageHelper.getInstance().getCurrentSize() > 0) {
                                finish.setText("完成(" + (checkedItems.size()+LocalImageHelper.getInstance().getCurrentSize())+")");
                                finish.setEnabled(true);
                                headerFinish.setText("完成(" + (checkedItems.size()+LocalImageHelper.getInstance().getCurrentSize())+")");
    ...
    public class MyAdapter extends BaseAdapter {
        ...
        public MyAdapter(Context context, List<LocalImageHelper.LocalFile> paths) {
            ...
            options=new DisplayImageOptions.Builder()
                    .cacheInMemory(true)
                    .cacheOnDisk(false)
                    .showImageForEmptyUri(R.mipmap.image_loadfail)
                    .showImageOnFail(R.mipmap.image_loadfail)
                    .showImageOnLoading(R.mipmap.image_loadfail)
